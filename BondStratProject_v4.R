...
Bonds_prices_10Y <- data.frame(matrix(NA, nrow = length(time_frame), ncol = 10))
colnames(Bonds_prices_10Y) <-c(Bond_names_10Y)
...
rownames(Bonds_prices_10Y) <- time_frame
...
for(i in Ticker_seq){
  ...
  Bonds_prices_10Y[[B_10Y_Tickers[i]]] <- coredata(zoo_interpolated_2)
